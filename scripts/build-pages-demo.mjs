...
const icon = (symbol) => `<span class="demo-icon" aria-hidden="true">${symbol}</span>`;
...
function home() {
  ...
  const body = `<main class="home-reference">${header()}
    ...
    <div class="hero-photo"><img src="${base}images/hero-team-reference.png" alt="Команда психологического центра"><div class="hero-quote">Помогаем<br>идти по-настоящему —<br>с опорой для бизнеса и семьи</div><a class="hero-play" href="${link("osnovatel")}"><span>▶</span><b>Посмотрите<br>о нашем подходе</b></a></div></section>
    <section class="path-strip"><div class="section-label">Наш путь</div><div class="path-content"><h2>Самый короткий путь к результату</h2><p>Научный подход. Личный маршрут. Гарантия результата.</p><div class="path-grid"><div class="path-item">${icon("✦")}<span>Научный подход</span></div><div class="path-item">${icon("♙")}<span>Специалист под запрос</span></div><div class="path-item">${icon("♢")}<span>Собственная методика</span></div><div class="path-item">${icon("▤")}<span>Поддержка при сложных случаях</span></div></div></div></section>
    <section class="pain-section section-shell"><h2>Всё хорошо снаружи. Но сил уже нет.</h2><div class="pain-grid"><div>${icon("✦")}<span>Тревога и напряжение</span></div><div>${icon("◷")}<span>Бизнес не отпускает</span></div><div>${icon("♧")}<span>Отдаление в семье</span></div><div>${icon("▥")}<span>Нет сил на близких</span></div><p class="hand-note">Ты можешь<br>по-другому ↙</p></div></section>
    ...
    <section class="income-section section-shell-wide"><h2>Доход растёт из устойчивого состояния</h2><div class="income-flow"><div>${icon("♨")}<span>Опора</span></div><b>→</b><div>${icon("◌")}<span>Решения</span></div><b>→</b><div>${icon("♧")}<span>Команда</span></div><b>→</b><div>${icon("↗")}<span>Доход</span></div><blockquote>«Устойчивость — не роскошь, а основа больших результатов»</blockquote></div></section>
...
css += `\n.hero-photo>img,.program-image>img,.specialist-photo>img,.reviews-photo>img,.interior-hero-image>img,.interior-card-image>img{position:absolute;inset:0;width:100%;height:100%}.demo-icon{width:38px;height:38px;display:inline-grid;place-items:center;border:1px solid #bda9a0;border-radius:50%;color:#7d0b24;font:22px Georgia,serif}.income-flow>b{font-weight:400;color:#9b7d84}.demo-menu{display:none;background:none;border:0;color:#7d0b24;font-size:25px}.demo-disabled{padding:35px;border:1px solid #ded5cf;border-radius:12px;background:#fffdfb}@media(max-width:980px){.demo-menu{display:block}.site-header>.button{display:none}.main-nav.open{display:flex;position:absolute;left:20px;right:20px;top:72px;padding:20px;flex-direction:column;background:#fff;box-shadow:0 15px 40px rgba(0,0,0,.12)}}`;
